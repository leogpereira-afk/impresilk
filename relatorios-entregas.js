...
const REL_ENT = {
 ano:new Date().getFullYear(), foco:String(new Date().getMonth()+1).padStart(2,'0'), selecionados:new Set(['entregue','vendido','recebido','funcionarios','retrabalho']), areasProducao:new Set(['Acabamento','Impressão digital','Montagem Interna','Operação de maquinas.','Serralheria','Instalação Externa']), dados:null,equipe:null,erro:'',carregando:false,mes:null,metrica:'entregue',pedido:0,
...
function renderRelatoriosEntregas() {
 ...
 <div id="rel-ent-imprimir">${r.erro?`<p role="alert" class="rel-ent-aviso">${esc(r.erro)} Use “Atualizar fontes” para tentar novamente.</p>`:''}${r.carregando?'<p role="status">Consultando histórico do servidor e do RH…</p>':''}${r.dados?`<p class="metricas-nota">${r.ano} · até ${relEntData(r.dados.ate)}. ${r.foco?'Resumo e tabelas: '+MES_CURTO[Number(r.foco)-1]+'. ':''}Gráficos: comparação anual. Clique nos pontos ou nos valores da tabela para conferir o mês. Lacunas significam informação indisponível, nunca zero.</p>
